...
accent structures rather than plain Unicode arrows when possible:
...
Matrices should be real editable `<m:m>` structures wrapped in OMML delimiters, not plain text brackets.

Summations and limits should use `<m:nary>` and `<m:limLow>` when limits are semantically attached.

Complex conjugates should use top bars. Vector notation should use OMML accents with the combining right arrow above:
...
- Use A4 with `2.54 cm` (`1 in`) margins on all sides by default.
...
4. `audit_docx_omml.py` for math documents
5. render to PDF/PNG for structural sanity when layout matters
...
- deterministic transcript-to-DOCX build
- accumulated rebuild from all previous batches
- two-section output: questions-only and repeated questions with solutions
- helper separation for Thai body runs, Thai labels, Latin text runs, footer runs, and OMML runs
- transcript validation before build
- centralized OMML primitives for `plain`, `expr`, `sup`, `sub`, `frac`, `rad`, `nary`, `lim_low`, `bar`, `func`, `delim`, `matrix`, `cases`, `thai_text`, and `acc`
- fail-fast behavior for unsupported math kinds and accidental Thai in generic math items
- QA command sequence embedded in the workflow, not left as memory

When a new failure mode appears, patch the shared builder/reference first, then rebuild affected files from source data.
